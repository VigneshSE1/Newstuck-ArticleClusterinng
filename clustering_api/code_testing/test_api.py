# -*- coding: utf-8 -*-
"""
Created on Mon Dec 21 10:46:01 2020
...
def getNewsTitlesFromJson(jsonData, key):
    ArrayOfSentence = []
    for data in jsonData:
        splittedSentence = clean_text(data[key])
...
def cluster_all():
    ...
    jsonTitles = english_titles
    unique_id = req_data['UUID']
    language = 'en'
    key = 'Title'
    daywise_result = []
    for idx, daywise_batch in enumerate(en_batches_daywise):
        for batch in daywise_batch:
            jsonTitles= batch
            vectorArray = getVectorsFromFastText(getNewsTitlesFromJson(batch, key), language)
